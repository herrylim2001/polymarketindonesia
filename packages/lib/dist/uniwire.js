 ...
 */
export function convertCryptoToIDR(cryptoAmount, cryptoCode) {
    let baseCurrency = cryptoCode.split('_')[0];
    if (baseCurrency === 'btc')
        baseCurrency = 'btc';
    if (cryptoCode.startsWith('usdt'))
 ...
 */
export function formatCryptoAmount(amount, cryptoCode) {
 ...
 */
export function getCryptoSymbol(cryptoCode) {
    const symbols = {
        btc: 'BTC',
        btc_lightning: 'BTC',
        eth: 'ETH',
 ...
 */
function generateSimulatedAddress(cryptoCode) {
    const prefixes = {
        bitcoin: '1',
        lightning: 'lnbc',
        ethereum: '0x',
 ...
 */
function getNetworkForCrypto(cryptoCode) {
    const networks = {
        btc: 'bitcoin',
 ...
 */
export async function createCryptoInvoice(amountIDR, cryptoCode, config = DEFAULT_CONFIG) {
 ...
 */
export function validateCallback(payload, signature, config = DEFAULT_CONFIG) {
 ...
 */
export function getEstimatedConfirmationTime(cryptoCode) {
    const times = {
        btc: '10-60 menit',
 ...
 */
export function getNetworkFeeEstimate(cryptoCode) {
    const fees = {
        btc: '~$2-10',
        btc_lightning: '< $0.01',
        eth: '~$1-5',
