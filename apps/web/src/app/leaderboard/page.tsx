...
export default function LeaderboardPage() {
  ...
  const { user, isLoggedIn, userBets } = useStore();

  useEffect(() => { setMounted(true); }, []);

  const leaderboardData = useMemo(() => {
    let entries = [...BASE_LEADERBOARD];

    const scale = period === 'weekly' ? 0.25 : period === 'monthly' ? 0.5 : 1;
    entries = entries.map(e => ({
      ...e,
      profit: Math.round(e.profit * scale),
      totalBets: Math.round(e.totalBets * scale),
    }));

    if (isLoggedIn && user && userBets.length > 0) {
      entries.push({
        rank: 0,
    ...
    return entries;
  }, [period, user, isLoggedIn, userBets]);
      ...
      {top3.length >= 3 && (
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="bg-dark-800 rounded-xl border border-dark-700 p-6 text-center mt-8">
            ...
            <p className="text-white font-semibold truncate">{top3[1].username}</p>
            <p className="text-green-500 font-bold mt-1">{formatIDR(top3[1].profit)}</p>
            <p className="text-dark-400 text-xs mt-1">{top3[1].winRate}% win rate</p>
          </div>
          ...
          <div className="bg-dark-800 rounded-xl border border-yellow-500/30 p-6 text-center relative">
            ...
            <p className="text-white font-bold text-lg truncate">{top3[0].username}</p>
            <p className="text-green-500 font-bold text-xl mt-1">{formatIDR(top3[0].profit)}</p>
            <p className="text-dark-400 text-sm mt-1">{top3[0].winRate}% win rate</p>
            <p className="text-dark-500 text-xs">{top3[0].totalBets} taruhan</p>
          </div>
          ...
          <div className="bg-dark-800 rounded-xl border border-dark-700 p-6 text-center mt-8">
            ...
            <p className="text-white font-semibold truncate">{top3[2].username}</p>
            <p className="text-green-500 font-bold mt-1">{formatIDR(top3[2].profit)}</p>
            <p className="text-dark-400 text-xs mt-1">{top3[2].winRate}% win rate</p>
          </div>
        </div>
      )}

      {isLoggedIn && user && userBets.length > 0 && (() => {
        const userEntry = leaderboardData.find(e => e.username === user.username);
        if (!userEntry) return null;
        return (
          <div className="bg-primary-500/10 border border-primary-500/20 rounded-xl p-4 mb-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-primary-400 font-bold text-lg">#{userEntry.rank}</span>
                <div className="w-10 h-10 bg-gradient-to-br from-primary-500 to-indigo-600 rounded-xl flex items-center justify-center text-white font-bold">
                  {userEntry.avatar}
                </div>
                <div>
                  <p className="text-white font-medium">{userEntry.username} <span className="text-primary-400 text-xs">(Anda)</span></p>
                  <p className="text-dark-400 text-sm">{userEntry.totalBets} taruhan | {userEntry.winRate}% win rate</p>
                </div>
              </div>
              <span className="text-green-500 font-bold">{formatIDR(userEntry.profit)}</span>
            </div>
          </div>
        );
      })()}
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-dark-700">
              {leaderboardData.map(trader => {
                const isCurrentUser = isLoggedIn && user?.username === trader.username;
                return (
                  <tr key={`${trader.rank}-${trader.username}`} className={`transition-colors ${
                    isCurrentUser ? 'bg-primary-500/5 hover:bg-primary-500/10' : 'hover:bg-dark-700/50'
                  }`}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-white font-medium">
                          {trader.username}
                          {isCurrentUser && <span className="text-primary-400 text-xs ml-1">(Anda)</span>}
                        </span>
                      </div>
                    </td>
